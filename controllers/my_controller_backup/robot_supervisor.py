...
BALANCE_REWARD_WEIGHT = 0 # 平衡奖励权重
PROGRESS_REWARD_WEIGHT = 500.0 # 接近目标奖励权重 (增加权重)
ANGULAR_VELOCITY_PENALTY_WEIGHT = 0.01 # 角速度惩罚权重，鼓励直线行走 (增加权重)
STILL_PENALTY = -1.0 # 静止惩罚，鼓励移动 (增加惩罚)
SURVIVAL_REWARD = 0.1 # 每一步存活奖励，鼓励机器人保持活跃
LIDAR_PROXIMITY_PENALTY_WEIGHT = 50.0 # Lidar近距离惩罚权重 (新增)
LIDAR_PROXIMITY_THRESHOLD = 1.0 # Lidar近距离惩罚阈值：当Lidar距离小于此值时开始惩罚 (新增)
...
# 步态控制参数
TARGET_TURN_GAIN = 3.0 # 根据目标角度调整转向幅度的增益
FORWARD_AMPLITUDE = 1.0 # 前进动作的步长幅度
TURN_AMPLITUDE = 0.4 # 转向动作的角速度幅度
TURN_FORWARD_AMPLITUDE = 0.2 # 转向时前进的步长幅度
...
class NavigationRobotSupervisor(RobotSupervisorEnv):
    ...
    def __init__(self, description, seed=None):
        ...
        OBSERVATION_SPACE_DIM = LIDAR_RESOLUTION + 3 + 3 + 3 + 2
        ...
        # ], dtype=np.float32)
        low_bounds = np.concatenate([
            np.full(LIDAR_RESOLUTION, 0.0),          # Lidar [0,1]
        ...
        high_bounds = np.concatenate([
            np.full(LIDAR_RESOLUTION, 1.0),          # Lidar
